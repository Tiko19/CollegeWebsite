...
namespace CollegeWebsite.Data
{
    internal class MasterDAO
    {
        ...
        public List<ProgramsModel> FetchAll()
        {
            ...
            {
                string loadQuery = "SELECT * FROM dbo.MasterPrograms";
        ...
        public ProgramsModel FetchOne(int id)
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                string loadQuery = "SELECT * FROM dbo.MasterPrograms WHERE Id = @Id";
        ...
        public int CreateOrUpdateRecord(ProgramsModel programsModel)
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                {
                    insertQuery = "INSERT INTO dbo.MasterPrograms Values(@Program, @Intake, @Duration, @Language, @Tuition)";
                }
                else
                {
                    insertQuery = "Update dbo.MasterPrograms SET Program = @Program, Intake = @Intake, Duration = @Duration, Language = @Language, Tuition = @Tuition WHERE Id = @Id";
                }
        ...
        internal int DeleteRecord(int id)
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                string deleteQuery = "DELETE FROM dbo.MasterPrograms WHERE Id = @id";

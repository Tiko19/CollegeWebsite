...
namespace CollegeWebsite.Data
{
    internal class BachelorDAO
    {
        ...
        public List<ProgramsModel> FetchAll()
        {
            ...
            {
                string loadQuery = "SELECT * FROM dbo.BachelorPrograms";
        ...
        public ProgramsModel FetchOne(int id)
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                string loadQuery = "SELECT * FROM dbo.BachelorPrograms WHERE Id = @Id";
        ...
        public int CreateOrUpdateRecord(ProgramsModel programsModel)
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                {
                    insertQuery = "INSERT INTO dbo.BachelorPrograms Values(@Program, @Intake, @Duration, @Language, @Tuition)";
                }
                else
                {
                    insertQuery = "UPDATE dbo.BachelorPrograms SET Program = @Program, Intake = @Intake, Duration = @Duration, Language = @Language, Tuition = @Tuition WHERE Id = @Id";
                }
        ...
        internal int DeleteRecord(int id)
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                string deleteQuery = "DELETE FROM dbo.BachelorPrograms WHERE Id = @id";
